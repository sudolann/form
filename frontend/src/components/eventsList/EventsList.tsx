...
export const EventsList: FunctionComponent = (): ReactElement => {
  ...
  useEffect((): void => {
    const fetchEvent = async (): Promise<void> => {
      try {
        const responseData = await sendRequest(`http://localhost:${process.env.REACT_APP_BACKEND_PORT}/events`);
  ...
  if(error) {
    return <ErrorAlert errorMessage="error with fetch events" fullPage/>
  }
  return (
    <ul className="list">
    {eventsList && (eventsList.length === 0 ? <ErrorAlert errorMessage="No events" fullPage/> : eventsList.map((event: EventPropsState)=> {
